...
contract FEEDVesting is Ownable {
    ...
    function tokensVestedPerMonth(uint256 _grantId) external view returns(uint256) {
        Grant storage tokenGrant = tokenGrants[_grantId];
        return tokenGrant.amount.div(uint256(tokenGrant.vestingDuration));
    }

    function getGrantInfo(uint256 _grantId) external view returns(uint256, uint256, uint256, uint256, uint256, uint256, address, address) {
        Grant storage tokenGrant = tokenGrants[_grantId];
        return (
